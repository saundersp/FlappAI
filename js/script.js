...
// Options AI (neat)
const nbBirds = 100; // Density of population
...
const keyInput = humanGame ? trackKeys(document, ['ArrowUp', 'ArrowLeft', 'ArrowRight', 'Space']) : 0;
...
const neu = getCanvas('neurons', {
	...
	coor: [-1, -1],
	onmousemove: (opt, e) => {
		const { left, top } = opt.canvas.getBoundingClientRect();
		opt.coor = [e.clientX - left, e.clientY - top];
	},
...
function getCanvas(name, opt) {
	...
	const res = { canvas, width, height, ctx: canvas.getContext('2d') };
...
function loadBrain() {
	const elt = $("input#dialog_file")[0];

	elt.onchange = async e => {
		const blob = e.target.files[0];
		if (blob.type !== "application/json") {
			alert("Invalid filetype");
			return;
		}
		bestBrainYet = JSON.parse(await blob.text());
	};
	elt.click();
}
...
function setupControlBoard() {
	...
	$("button#load_brain").click(loadBrain);
}
...
function saveBlobAsFile(name, content) {
	...
	dl.download = name;
	dl.href = URL.createObjectURL(new Blob([content], {
		type: 'text/json'
	}));
